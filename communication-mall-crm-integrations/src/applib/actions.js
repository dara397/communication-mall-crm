...
import { providerBySlug } from './integrations';
import { getIntegrationRaw } from './db';

const intgOk = (m) => redirect('/integrations?ok=' + encodeURIComponent(m));
const intgErr = (m) => redirect('/integrations?err=' + encodeURIComponent(m));

const AUTH_SET = new Set(['api_key', 'bearer', 'basic', 'none']);

export async function saveIntegration(formData) {
  await requireAdmin();

  const id = str(formData.get('id'));
  const name = str(formData.get('name'));
  if (!name) intgErr('Give this connection a name so you can recognise it.');

  const providerSlug = str(formData.get('provider')) || 'custom';
  const preset = providerBySlug(providerSlug);
  let authType = str(formData.get('authType'));
  if (!AUTH_SET.has(authType)) authType = preset.authType || 'api_key';

  const data = {
    name,
    provider: providerSlug,
    authType,
    baseUrl: str(formData.get('baseUrl')),
    username: str(formData.get('username')),
    notes: str(formData.get('notes')),
    active: formData.get('active') != null,
  };

  // Secrets: a blank field on an existing record means "keep what's there".
  const newKey = String(formData.get('apiKey') || '');
  const newSecret = String(formData.get('apiSecret') || '');

  if (id) {
    if (newKey.trim() !== '') data.apiKey = encryptSecret(newKey.trim());
    if (newSecret.trim() !== '') data.apiSecret = encryptSecret(newSecret.trim());
    await prisma.integration.update({ where: { id }, data });
    revalidatePath('/integrations');
    intgOk(`${name} updated.`);
  } else {
    data.apiKey = newKey.trim() ? encryptSecret(newKey.trim()) : '';
    data.apiSecret = newSecret.trim() ? encryptSecret(newSecret.trim()) : '';
    await prisma.integration.create({ data });
    revalidatePath('/integrations');
    intgOk(`${name} connected. Use “Test” to confirm the credentials work.`);
  }
}

export async function deleteIntegration(formData) {
  await requireAdmin();
  const id = str(formData.get('id'));
  if (id) await prisma.integration.delete({ where: { id } });
  revalidatePath('/integrations');
  intgOk('Connection removed.');
}

export async function toggleIntegration(formData) {
  await requireAdmin();
  const id = str(formData.get('id'));
  const row = await prisma.integration.findUnique({ where: { id } });
  if (row) {
    await prisma.integration.update({ where: { id }, data: { active: !row.active } });
  }
  revalidatePath('/integrations');
}

/**
 * Test a connection by making a lightweight GET to its base URL with the
 * stored credentials. We treat any HTTP response (even 401/403) as "we reached
 * the platform"; a network failure or timeout is the real failure. The result
 * is saved so the status shows on the page.
 */
export async function testIntegration(formData) {
  await requireAdmin();
  const id = str(formData.get('id'));
  const row = await getIntegrationRaw(id);
  if (!row) intgErr('Could not find that connection.');
  ...
  if (!baseUrl) {
    await prisma.integration.update({
      ...
      data: {
        ...
        lastTestOk: false,
        lastTestMessage: 'No base URL set — add the platform’s API URL to test it.',
      },
    });
    revalidatePath('/integrations');
    intgErr('Add a base URL for this platform before testing.');
  }

  const key = decryptSecret(row.apiKey);
  const secret = decryptSecret(row.apiSecret);
  const headers = { Accept: 'application/json' };
  if (row.authType === 'bearer' && key) headers.Authorization = `Bearer ${key}`;
  else if (row.authType === 'api_key' && key) {
    headers.Authorization = `Bearer ${key}`;
    headers['X-API-Key'] = key;
  } else if (row.authType === 'basic' && (row.username || key)) {
    const basic = Buffer.from(`${row.username}:${secret || key}`).toString('base64');
  ...
  try {
    ...
    ok = true; // we reached the server and got an HTTP response
    if (res.status === 401 || res.status === 403) {
      message = `Reached the platform, but it rejected the credentials (HTTP ${res.status}). Double-check the key.`;
      ok = false;
    } else {
      message = `Reached the platform (HTTP ${res.status}).`;
    }
  } catch (e) {
    ok = false;
    message =
      e?.name === 'AbortError'
        ? 'The platform did not respond within 8 seconds.'
        : `Could not reach the platform: ${e?.message || 'network error'}.`;
  } finally {
    clearTimeout(timer);
  }
  ...
  revalidatePath('/integrations');
  if (ok) intgOk(message);
  else intgErr(message);
}
